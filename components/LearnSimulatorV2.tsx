...
export function LearnSimulatorV2({ initialScenarioId }: { initialScenarioId?: string }) {
  ...
  useEffect(() => {
    ...
    if (saved && !saved.completed && saved.maxStep > 0) setStepIndex(Math.min(saved.maxStep, Math.max(scenario.steps.length - 1, 0)));
  ...
  const modeLabel = mode === "learn"
    ? complete ? t("learn.learningComplete") : t("learn.stepOf", { current: stepIndex + 1, total: scenario.steps.length })
    : mode === "challenge" ? t("learn.bossOf", { current: challengeIndex + 1, total: scenario.challenges.length })
      : t("learn.bossResult");
  const modeColor = mode === "challenge" ? "amber" : mode === "results" ? "cyan" : "emerald";

  return (
    <main className="min-h-screen pb-16">
      <LearnerNav active="learn" eyebrow={t("learn.navEyebrow")} />
      <div className="mx-auto max-w-[1440px] px-4 md:px-8">
        <section className="py-4 md:py-5">
          <div className="flex items-center gap-2 md:hidden">
            <label className="min-w-0 flex-1">
              <span className="sr-only">{t("learn.chooseScenario")}</span>
            ...
            <Link href="/scenarios" className="button-ghost shrink-0">{t("learn.browse")}</Link>
          </div>

          <div className="no-scrollbar hidden gap-1 overflow-x-auto rounded-2xl border border-white/[0.065] bg-black/15 p-1.5 md:flex">
            {learningScenarios.map((item) => <button key={item.id} onClick={() => resetScenario(item.id)} className={`shrink-0 rounded-xl px-3.5 py-2 text-xs font-semibold transition ${item.id === scenario.id ? "bg-white/10 text-white shadow-sm" : "text-white/34 hover:bg-white/[0.035] hover:text-white/68"}`}>{item.shortTitle}</button>)}
            <Link href="/scenarios" className="ml-auto shrink-0 rounded-xl px-3.5 py-2 text-xs font-semibold text-emerald-200/55 transition hover:bg-emerald-300/[0.05] hover:text-emerald-100">{t("learn.browseAll")}</Link>
          </div>
        </section>
          ...
          <div className="relative grid gap-5 xl:grid-cols-[minmax(0,1fr)_430px] xl:items-end">
            <div>
              ...
              <div className="mt-4 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-white/28">
                {progressReady && persistedScenario && <span>{t("learn.fixesSaved", { solved: persistedScenario.solvedChallenges.length, total: scenario.challenges.length })}</span>}
                {bossScoresReady && bestBossScore !== undefined && <span>{t("learn.bestBoss", { score: bestBossScore })}</span>}
              </div>
            </div>
              ...
              {mode === "learn" && (
                <div className="grid gap-2 sm:grid-cols-[auto_1fr] xl:grid-cols-1">
                  <button onClick={() => setSimpleMode((value) => !value)} className={`rounded-2xl border px-4 py-3 text-left transition ${simpleMode ? "border-sky-300/24 bg-sky-300/[0.07] text-sky-100" : "border-white/8 bg-black/15 text-white/45 hover:border-white/14 hover:text-white/70"}`}>
                    <span className="block text-xs font-semibold">{simpleMode ? t("learn.beginnerOn") : t("learn.newToDataspaces")}</span>
                    <span className={`mt-0.5 block text-[10px] ${simpleMode ? "text-sky-100/45" : "text-white/25"}`}>{simpleMode ? t("learn.jargonHidden") : t("learn.plainEnglish")}</span>
                  </button>
                  {!simpleMode && <DepthSwitcher value={depth} onChange={setDepth} />}
                </div>
              )}
              <div>
                <div className="flex items-center justify-between text-[10px] font-semibold uppercase tracking-[0.13em] text-white/25"><span>{mode === "learn" ? t("learn.learningProgress") : mode === "challenge" ? t("learn.diagnosticRun") : t("learn.completed")}</span><span>{mode === "learn" ? `${progress}%` : "100%"}</span></div>
        ...
        {mode === "learn" ? (
          <>
            <section className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.08fr)_minmax(410px,.92fr)] xl:items-start">
                ...
                {complete ? (
                  ...
                  <>
                    <div className="flex items-start justify-between gap-3">
                      <div><p className="eyebrow">{t("learn.nowStep", { step: stepIndex + 1 })}</p><p className="mt-1 text-[11px] text-white/25">{simpleMode ? t("learn.plainMode") : depthLabel(depth, t)}</p></div>
                      <span className="max-w-[45%] truncate rounded-xl border border-white/8 bg-black/15 px-2.5 py-1.5 text-[10px] font-semibold text-white/38">{simpleMode ? t("learn.noJargon") : step.technicalName}</span>
                    </div>
                    ...
                    <div className={`mt-5 rounded-2xl border p-4 md:p-5 ${simpleMode ? "border-sky-300/13 bg-sky-300/[0.045]" : "border-emerald-300/13 bg-emerald-300/[0.045]"}`}>
                      <p className={`text-[10px] font-semibold uppercase tracking-[0.16em] ${simpleMode ? "text-sky-200/50" : "text-emerald-200/50"}`}>{simpleMode ? t("learn.whatMeans") : t("learn.explanation")}</p>
                    ...
                    <div className="mt-4 grid gap-2 sm:grid-cols-2">
                      <details className="rounded-2xl border border-white/8 bg-black/12 p-3.5 open:bg-white/[0.02]"><summary className="flex cursor-pointer items-center justify-between gap-2 text-xs font-semibold text-white/58"><span>{t("learn.whyNeeded")}</span><span className="text-white/22">+</span></summary><p className="mt-3 text-xs leading-6 text-white/38">{step.whyNeeded}</p></details>
                      <details className="rounded-2xl border border-white/8 bg-black/12 p-3.5 open:bg-white/[0.02]"><summary className="flex cursor-pointer items-center justify-between gap-2 text-xs font-semibold text-white/58"><span>{t("learn.whatBreaks")}</span><span className="text-white/22">+</span></summary><p className="mt-3 text-xs leading-6 text-white/38">{step.withoutIt}</p></details>
                    </div>

                    {!simpleMode && <div className="mt-3"><Glossary terms={step.glossary} /></div>}

                    <div className="mt-6 grid grid-cols-[auto_1fr] gap-2">
                      <button onClick={() => setStepIndex((value) => previousStepIndex(value))} disabled={stepIndex === 0} className="rounded-xl border border-white/9 px-4 py-3 text-sm font-semibold text-white/45 transition hover:border-white/15 hover:text-white/72 disabled:cursor-not-allowed disabled:opacity-25">{t("learn.back")}</button>
                      <button onClick={() => setStepIndex((value) => nextStepIndex(value, scenario.steps.length))} className="rounded-xl bg-emerald-300 px-5 py-3 text-sm font-bold text-[#07110f] shadow-[0_12px_30px_rgba(110,231,183,.1)] transition hover:-translate-y-0.5 hover:bg-emerald-200">{simpleMode ? t("learn.continue") : step.actionLabel} →</button>
                    </div>
                  </>
                )}
              </aside>
              ...
              <details className="surface-card overflow-hidden xl:hidden">
                <summary className="flex cursor-pointer items-center justify-between gap-4 p-4 text-sm font-semibold text-white/62"><span><span className="mr-2 text-cyan-300">◇</span>{t("learn.openVisual")}</span><span className="text-xs font-normal text-white/28">{t("learn.mapTimeline")}</span></summary>
            ...
            {!complete && (
              <section className="mt-4">
                <div className="mb-2 flex items-center justify-between px-1"><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-white/25">{t("learn.jumpToStep")}</p><span className="text-[10px] text-white/20">{t("learn.jumpHint")}</span></div>
                <div className="no-scrollbar flex gap-2 overflow-x-auto pb-2">
                  {scenario.steps.map((item, index) => (
                    <button key={item.id} onClick={() => setStepIndex(index)} className={`min-w-[150px] shrink-0 rounded-2xl border p-3 text-left transition md:min-w-[175px] ${index < stepIndex ? "border-emerald-300/15 bg-emerald-300/[0.035] text-emerald-100/62" : index === stepIndex ? "border-white/18 bg-white/[0.045] text-white shadow-[0_12px_36px_rgba(0,0,0,.1)]" : "border-white/7 bg-black/10 text-white/27 hover:border-white/12"}`}>
                      <span className="flex items-center justify-between text-[9px] font-semibold uppercase tracking-[0.12em] opacity-55"><span>{t("learn.stepShort", { step: index + 1 })}</span><span>{index < stepIndex ? "✓" : index === stepIndex ? t("learn.now") : ""}</span></span>
          ...
          <section className="mt-5 grid gap-5 xl:grid-cols-[.78fr_1.22fr] xl:items-start">
            ...
            <aside className="rounded-[1.75rem] border border-amber-300/14 bg-gradient-to-b from-amber-300/[0.035] to-white/[0.015] p-5 md:p-7">
              <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-amber-300">{t("learn.bossDiagnose")}</p><span className="rounded-xl border border-amber-300/12 bg-black/15 px-2.5 py-1 text-[10px] text-amber-100/50">{t("learn.liveScore", { score: liveBossScore.score })}</span></div>
              ...
              <div className="mt-5 space-y-2.5">
                {challenge.options.map((option) => {
                  ...
                  return <button key={option.id} onClick={() => chooseChallengeOption(option.id)} className={`w-full rounded-2xl border p-4 text-left transition ${selected ? challengeResult?.correct ? "border-emerald-300/35 bg-emerald-300/[0.075]" : "border-rose-300/28 bg-rose-300/[0.055]" : "border-white/8 bg-black/10 hover:border-white/15 hover:bg-white/[0.025]"}`}><div className="flex items-center justify-between gap-3"><span className="text-sm font-semibold md:text-base">{option.label}</span>{selected && <span className={challengeResult?.correct ? "text-emerald-300" : "text-rose-300"}>{challengeResult?.correct ? "✓" : "×"}</span>}</div>{selected && <p className="mt-2 text-xs leading-6 text-white/42 md:text-sm">{option.explanation}</p>}</button>;
                })}
              </div>

              {!challengeResult?.correct && <button onClick={revealHint} className="mt-4 rounded-xl border border-amber-300/10 px-3 py-2 text-xs font-semibold text-amber-200/58 transition hover:bg-amber-300/[0.04] hover:text-amber-100">{t("learn.revealHint")}</button>}
              {showHint && !challengeResult?.correct && <p className="mt-3 rounded-2xl border border-amber-300/9 bg-amber-300/[0.045] p-4 text-sm leading-6 text-amber-100/58">{t("learn.hint", { hint: challenge.hint })}</p>}

              {challengeResult?.correct && <div className="mt-5 rounded-2xl border border-emerald-300/16 bg-emerald-300/[0.055] p-4"><p className="text-sm font-semibold text-emerald-200">{t("learn.rootFound")}</p><p className="mt-2 text-sm leading-6 text-white/48">{challenge.rootCause}</p><button onClick={nextChallenge} className="mt-4 rounded-xl bg-emerald-300 px-5 py-2.5 text-sm font-bold text-[#07110f]">{challengeIndex === scenario.challenges.length - 1 ? t("learn.seeResult") : t("learn.nextFailure")} →</button></div>}

              <details className="mt-4 rounded-2xl border border-white/7 bg-black/10 p-3 xl:hidden"><summary className="cursor-pointer text-xs font-semibold text-white/42">{t("learn.openMapScore")}</summary><div className="mt-3 space-y-3"><DataspaceMap focus={["supplier-edc", "dataspace", "consumer-edc"]} direction="both" /><BossScoreCard score={liveBossScore.score} wrongAttempts={wrongAttempts} hintsUsed={hintsUsed} /></div></details>
            </aside>
          </section>
...
function depthLabel(depth: LearningDepth, t: Translate) {
  if (depth === "business") return t("learn.managerDepth");
  if (depth === "architecture") return t("learn.architectDepth");
  return t("learn.developerDepth");
}
...
function SimpleStoryMap({ supplier, manufacturer }: { supplier: string; manufacturer: string }) {
  ...
  return (
    <div className="surface-card p-4 md:p-6">
      <div className="flex items-center justify-between"><div><p className="eyebrow !text-sky-300">{t("learn.simpleStory")}</p><p className="mt-1 text-xs text-white/28">{t("learn.simpleStoryIntro")}</p></div><span className="rounded-xl border border-sky-300/12 bg-sky-300/[0.04] px-2.5 py-1 text-[10px] text-sky-100/50">{t("learn.beginnerView")}</span></div>
      <div className="mt-5 grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2 md:gap-4">
        <div className="rounded-2xl border border-white/8 bg-black/15 p-3 text-center md:p-4"><div className="mx-auto grid h-9 w-9 place-items-center rounded-xl bg-white/[0.04] text-lg">P</div><p className="mt-2 truncate text-xs font-semibold md:text-sm">{supplier}</p><p className="mt-1 text-[10px] text-white/28">{t("learn.hasUsefulData")}</p></div>
        <div className="text-center"><p className="hidden text-[9px] font-semibold uppercase tracking-[0.12em] text-white/24 sm:block">{t("learn.trustedExchange")}</p><p className="mt-1 text-2xl text-sky-300 md:text-3xl">↔</p></div>
        <div className="rounded-2xl border border-white/8 bg-black/15 p-3 text-center md:p-4"><div className="mx-auto grid h-9 w-9 place-items-center rounded-xl bg-white/[0.04] text-lg">C</div><p className="mt-2 truncate text-xs font-semibold md:text-sm">{manufacturer}</p><p className="mt-1 text-[10px] text-white/28">{t("learn.needsData")}</p></div>
      </div>
    </div>
  );
}
...
function BossScoreCard({ score, wrongAttempts, hintsUsed }: { score: number; wrongAttempts: number; hintsUsed: number }) {
  ...
  return (
    <div className="surface-card p-5">
      <div className="flex items-end justify-between gap-4"><div><p className="eyebrow !text-amber-300">{t("learn.liveScore", { score: "" }).replace(" /100", "").replace("/100", "")}</p><p className="mt-2 text-4xl font-semibold tracking-[-0.04em]">{score}<span className="text-base text-white/24">/100</span></p></div><p className="whitespace-pre-line text-right text-[10px] leading-5 text-white/27">{t("learn.scoreWrongHint")}</p></div>
      <div className="mt-4 grid grid-cols-2 gap-2 text-xs"><div className="rounded-xl border border-white/6 bg-black/12 p-3"><span className="text-white/28">{t("learn.wrongAttempts")}</span><p className="mt-1 text-lg font-semibold">{wrongAttempts}</p></div><div className="rounded-xl border border-white/6 bg-black/12 p-3"><span className="text-white/28">{t("learn.hintsUsed")}</span><p className="mt-1 text-lg font-semibold">{hintsUsed}</p></div></div>
    </div>
  );
}
...
function CompletionCard({ scenarioTitle, onRestart, onBossFight, hasChallenges, bestScore }: { scenarioTitle: string; onRestart: () => void; onBossFight: () => void; hasChallenges: boolean; bestScore?: number }) {
  ...
  return (
    <div className="flex min-h-[390px] flex-col justify-center">
      ...
      <p className="mt-5 text-[10px] font-bold uppercase tracking-[0.17em] text-emerald-300">{t("learn.flowComplete")}</p>
      <h2 className="mt-3 max-w-xl text-3xl font-semibold leading-tight tracking-[-0.035em]">{t("learn.completionHeadline", { scenario: scenarioTitle })}</h2>
      <p className="mt-3 max-w-lg text-sm leading-6 text-white/40">{t("learn.completionText")}</p>
      {bestScore !== undefined && <p className="mt-4 text-xs font-semibold text-cyan-200/58">{t("learn.personalBest", { score: bestScore })}</p>}
      <div className="mt-7 flex flex-wrap gap-2">{hasChallenges && <button onClick={onBossFight} className="rounded-xl bg-amber-300 px-5 py-3 text-sm font-bold text-[#171207]">{t("learn.startBoss")}</button>}<button onClick={onRestart} className="rounded-xl border border-white/9 px-5 py-3 text-sm font-semibold text-white/52 transition hover:text-white/75">{t("learn.runAgain")}</button></div>
    </div>
  );
}
...
function BossResultCard({ result, bestScore, onRetry, onLearn }: { result: BossFightResult; bestScore: number; onRetry: () => void; onLearn: () => void }) {
  ...
  return (
    <section className="surface-hero mx-auto mt-6 max-w-4xl overflow-hidden p-6 text-center md:p-10">
      <p className="eyebrow !text-cyan-300">{t("learn.bossComplete")}</p>
      ...
      <p className="mx-auto mt-4 max-w-xl text-sm leading-7 text-white/42">{t("learn.bossCompleteText")}</p>
      <div className="mx-auto mt-7 grid max-w-lg grid-cols-3 gap-2 text-sm"><ResultMetric label={t("learn.wrong")} value={result.wrongAttempts} /><ResultMetric label={t("learn.hints")} value={result.hintsUsed} /><ResultMetric label={t("learn.best")} value={bestScore} /></div>
      <div className="mt-8 flex flex-wrap justify-center gap-2"><button onClick={onRetry} className="rounded-xl bg-cyan-300 px-5 py-3 text-sm font-bold text-[#07110f]">{t("learn.retryScore")}</button><button onClick={onLearn} className="rounded-xl border border-white/9 px-5 py-3 text-sm font-semibold text-white/50">{t("learn.backLearning")}</button></div>
    </section>
  );
}
