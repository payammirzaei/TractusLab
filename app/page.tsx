        ...
        <section className="grid min-h-[76vh] items-center gap-8 py-10 md:py-16 lg:grid-cols-[1.02fr_.98fr] lg:gap-12">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-2.5">
              <Link href="/path" className="rounded-xl bg-emerald-300 px-5 py-3.5 text-sm font-bold text-white shadow-[0_14px_38px_rgba(15,159,118,.16)] transition hover:-translate-y-0.5 hover:bg-emerald-600">Start guided path →</Link>
              <Link href="/scenarios" className="rounded-xl border border-blue-200 bg-blue-50 px-5 py-3.5 text-sm font-semibold text-blue-700 transition hover:border-blue-300 hover:bg-blue-100">Explore scenarios</Link>
            </div>
          ...
          <div className="surface-hero relative overflow-hidden p-4 md:p-6">
            ...
            <div className="relative">
              <div className="flex items-center justify-between"><div><p className="eyebrow">Live learning preview</p><p className="mt-1 text-xs text-white/28">Battery PCF · Manager view</p></div><span className="rounded-xl border border-blue-200 bg-blue-50 px-2.5 py-1 text-[10px] font-semibold text-blue-700">Step 2 / 7</span></div>
              ...
              <div className="mt-6 grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2 md:gap-4">
                ...
                <div className="text-center"><span className="hidden text-[9px] font-bold uppercase tracking-[0.12em] text-white/22 sm:block">governed exchange</span><div className="mt-1 text-3xl text-blue-600">↔</div></div>
                <DemoCompany label="Manufacturer" detail="Needs PCF data" symbol="C" />
              </div>

              <div className="mt-5 rounded-2xl border border-blue-100 bg-blue-50/60 p-4">
                <div className="flex items-center justify-between"><p className="text-[9px] font-bold uppercase tracking-[0.14em] text-blue-700">Now · Catalog discovery</p><span className="text-[10px] text-white/24">Business meaning</span></div>
              ...
              <div className="mt-4 grid grid-cols-5 gap-1.5">
                {["Identity", "Catalog", "Policy", "Agreement", "Transfer"].map((item, index) => <div key={item} className={`rounded-xl border px-2 py-2 text-center text-[9px] font-semibold ${index === 0 ? "border-emerald-200 bg-emerald-50 text-emerald-700" : index === 1 ? "border-blue-200 bg-blue-50 text-blue-700" : "border-white/6 text-white/20"}`}>{index === 0 ? "✓ " : ""}{item}</div>)}
              </div>
            </div>
          </div>
        </section>
