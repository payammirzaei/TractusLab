...
export default function LearnPage() {
  ...
  const scenario = useMemo(
    () => learningScenarios.find((item) => item.id === scenarioId) ?? learningScenarios[0],
    [scenarioId],
  );
  ...
  const challengeResult = selectedOption ? evaluateChallenge(challenge, selectedOption) : null;
  ...
  return (
    <main className="min-h-screen px-4 py-5 md:px-8 md:py-7">
      <div className="mx-auto max-w-7xl">
            ...
            {learningScenarios.map((item) => (
              <button
                ...
                className={`rounded-full border px-3 py-1.5 text-xs transition ${
                  item.id === scenario.id ? "border-emerald-300/45 bg-emerald-300/10 text-emerald-100" : "border-white/10 text-white/40 hover:text-white/70"
                }`}
              >
                {item.shortTitle}
              </button>
        ...
        {mode === "learn" ? (
          <section className="mt-6 grid gap-6 xl:grid-cols-[1.05fr_.95fr]">
            ...
            <aside className="rounded-[2rem] border border-white/10 bg-[#0b1714] p-5 md:p-7">
              {complete ? (
                <CompletionCard scenarioTitle={scenario.shortTitle} onRestart={() => resetScenario()} onChallenge={startChallenges} />
              ) : (
                <>
            ...
            <aside className="rounded-[2rem] border border-amber-300/20 bg-[#17150b] p-5 md:p-7">
              ...
              <div className="mt-5 space-y-3">
                {challenge.options.map((option) => {
                  ...
                  const optionCorrect = option.id === challenge.correctOptionId;
                  return (
                    <button
                      key={option.id}
                    ...
                    >
                      <div className="flex items-center justify-between gap-3">
                        ...
                        {selected && <span>{optionCorrect ? "✓" : "×"}</span>}
                      </div>
        ...
        {mode === "learn" && !complete && (
          <section className="mt-6 grid gap-2 sm:grid-cols-2 lg:grid-cols-7">
...
function CompletionCard({ scenarioTitle, onRestart, onChallenge }: { scenarioTitle: string; onRestart: () => void; onChallenge: () => void }) {
  return (
    <div className="flex min-h-[560px] flex-col justify-center">
      ...
      <p className="mt-5 leading-7 text-white/58">Next, break the simulated dataspace and diagnose what failed. That is where the concepts become operational knowledge.</p>
      <div className="mt-7 flex flex-wrap gap-3">
        <button onClick={onChallenge} className="rounded-full bg-amber-300 px-6 py-2.5 text-sm font-semibold text-[#171207]">Start Break & Fix →</button>
